...
class DigitalClock {
  ...
  renderTzDropdownList() {
    this.tzListEl.innerHTML = this.availableTimezones.map(item => `
    ...
    this.tzListEl.querySelectorAll('input[type="checkbox"]').forEach(cb => {
      cb.addEventListener('change', (e) => {
        ...
        localStorage.setItem('js_clock_selected_tz', JSON.stringify(this.selectedTzIds));
        this.update();
      });
    });
  }
  ...
  update() {
    ...
    const selectedTzObjects = this.availableTimezones.filter(item => this.selectedTzIds.includes(item.id));
...
function openGoogleCalendar({ title, dateStr, timeStr, timeZoneStr, details }) {
  ...
  const endDateObj = isNaN(startDateObj.getTime()) ? new Date() : new Date(startDateObj.getTime() + 30 * 60000);
...
class AlarmManager {
      ...
      cb.addEventListener('change', (e) => {
        ...
        if (e.target.checked) {
          ...
          if (this.formSelectedTzIds.length > 1) {
            this.formSelectedTzIds = this.formSelectedTzIds.filter(tId => tId !== id);
          } else {
            e.target.checked = true;
            return;
          }
  ...
  startAudioSound() {
    try {
      ...
      this.alarmOscillator = setInterval(() => {
        ...
        osc.type = 'square';
        osc.frequency.setValueAtTime(toneHigh ? 880 : 587.33, this.audioCtx.currentTime);
        toneHigh = !toneHigh;
  ...
  render() {
    ...
    this.alarmsListEl.innerHTML = this.alarms.map(a => {
      ...
      const selectedObjs = this.availableTimezones.filter(item => tzIds.includes(item.id) || tzIds.includes(item.tz));
      ...
      if (!isMulti) {
        ...
        const pills = selectedObjs.map(item => {
          const convertedTime = this.computeConvertedTime(a.date, a.time, item.tz);
          return `
